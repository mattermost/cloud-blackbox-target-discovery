...
CLOUD_BLACKBOX_TARGET_DISCOVERY_IMAGE ?= mattermost/cloud-blackbox-target-discovery:test
...
.PHONY: build
build: ## Build the cloud-blackbox-target-discovery
	@echo Building Cloud-Blackbox-Target-Discovery
...
build-image:  ## Build the docker image for cloud-blackbox-target-discovery
	@echo Building Cloud-Blackbox-Target-Discovery Docker Image
	docker build \
	--build-arg DOCKER_BUILD_IMAGE=$(DOCKER_BUILD_IMAGE) \
	--build-arg DOCKER_BASE_IMAGE=$(DOCKER_BASE_IMAGE) \
	. -f build/Dockerfile -t $(CLOUD_BLACKBOX_TARGET_DISCOVERY_IMAGE) \
	--no-cache
